...
## Data Preparation & Variable Selection
<br>
We first restricted the dataset to the variables needed for BMI, physical activity, age, and health status indicators. Variables were recoded into appropriate factor formats and ordered where meaningful (e.g., interview year, BMI category, age group, education). We also created several binary indicators to support modeling: obesity status (`obese`), exercise (`exercise`), diabetes (`diabetes`), heart attack history (`heart_attack`), coronary heart disease history (`coronary`), stroke history (`stroke`), and self-reported health (`self_health`). These transformations ensure consistent interpretation across regression and classification analyses.
<br>
...
brfss_1 = brfss[,c(5:8,13:14,19:22,24)]
...
```{r, message=FALSE, warning=FALSE}
brfss_nums = brfss_1[,c(1,5,12:18)]
str(brfss_nums)
...
```
<br>
The correlation matrix does not show a strong correlation between BMI and exercise. In the matrix we do see stronger correlation between BMI & age and BMI & diabetes. This suggests that these variables may be important our model.
...
<br> To evaluate categorical relationships, we created a contingency table between physical activity and BMI category and performed a chi-squared test. This tests whether BMI category distributions differ between physically active and inactive groups. The second contingency table between physical activity and overweight/obesed and performed a chi-squared test. This tests whether the obese/overweight category distributions differ between physically active and inactive groups. This will be helpful in our logistic regression model. <br>
...
bmi_activitytable2 = xtabs(~ any_exercise_last_month + overweight_or_obese, data = brfss_1)
bmi_activitytable2

chisqactivity2 = chisq.test(bmi_activitytable2)
chisqactivity2
```
<br> A small p-values in both chi-squared tests supports the conclusion that BMI category and physical activity are not independent, meaning exercise status is associated with BMI classification categories. This will be helpful in our logistic regression model.<br>

## Chi-Squared Test: Self-Reported Health vs BMI Category & Overweight/Obesed
...
```
<br> A small p-values in both chi-squared tests supports the conclusion that BMI category and self health are not independent, meaning exercise status is associated with BMI classification categories. <br>
...
```
<br> A significant t-test indicates a meaningful difference in average BMI between exercise groups, and the group means provide the direction of that difference. From looking at the below mean body mass index of participants who reported being physically active in the last 30 days vs participants who did not report being physically active, we can observe that the average body mass indexes are different.
...
```
<br>
This model estimates the mean difference in BMI between the exercise groups. A statistically significant exercise coefficient would suggest that physically active adults have a different average BMI than inactive adults, but this model does not yet adjust for age or health status. Also, based on the R-squared value the model explains 1.5% of the variation within BMI.
<br>

## Linear Regression with Covariates
<br>
To reduce confounding, we expanded the model to include age and self-reported health. This allows us to test whether the association between exercise and BMI persists after accounting for important age demographics and health differences.
<br>
...
```
<br> Including the age and self health covariates helps interpret the exercise coefficient as the estimated association with BMI. Based on the variable p-values, each variable is significant in the model. Also, based on the R-squared value the model explains 3.5% of the variation within BMI, an increase from the model that just included exercise.<br>

## Expanded Linear Model
4
<br> We further included additional health indicators: diabetes, stroke history, heart attack history, and coronary heart disease history. This model assesses whether exercise remains associated with BMI even after controlling for comorbidity patterns that are strongly linked with weight status. <br>

```{r, message=FALSE, warning=FALSE}
fit3 <- lm(bmi ~ exercise + age + self_reported_health + diabetes + stroke + heart_attack +coronary, data = brfss_1)
summary(fit3)
xkablevif(fit3)
plot(fit3)
```
<br> This model including all physical health factors shows that all of the variables are significant in predicting BMI, except heart attack history (heart_attack) and coronary heart disease history (coronary). We decided to remove the non-significant variable from our model and found that with the new model explains 5.3% of the variation within BMI. 
<br>
```{r, message=FALSE, warning=FALSE}
fit4 <- lm(bmi ~ exercise + age + self_reported_health + diabetes + stroke, data = brfss_1)
summary(fit4)
xkablevif(fit4)
plot(fit4)
```
...
```
<br> A significant interaction term would indicate that the association between predictors is not constant across groups (e.g., exercise relates differently to BMI for people with poor vs good health). 
<br>
<br>
Looking at our linear models, we can conclude that the strongest models includes exercise, age, self reported health, diabetes status, and stroke history when predicting BMI. 
<br>
<br>
## Logistic Regression & ROC/AUC (Classification View)
<br> In addition to modeling BMI directly, we used logistic regression to examine predictive ability in a classification setting. We first modeled physical activity status from BMI (and additional covariates), and evaluated performance using ROC curves and AUC values. In the logistic regressions model for obese, 0 represents obese and 1 represents not obese. <br>
```{r, message=FALSE, warning=FALSE}
